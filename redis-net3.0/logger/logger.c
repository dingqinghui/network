...
char* on_level_to_str(int level){
    switch (level)
    {
    case NLOG_LVL_DEBUG :
        return "DEBUG";
...
void log_writer_init(nlog_writer* writer ,const char* path,int level,long long  max_file_size){
    ...
    const char* str = on_level_to_str(level);
    memcpy(writer->str_lv,str,sizeof(str));
}
...
void log_writer_create_file(nlog_writer* writer){
    ...
    sprintf(filepath,"%s%s_%02d%02d%02d_%d",filepath,writer->str_lv,
...
static int on_format_time_info(char* buffer,int bufsize, int l)
{
	...
	struct tm vtm; 
    localtime_r(&now, &vtm);
    return snprintf(buffer, bufsize, "[%s]:[%d-%02d-%02d %02d:%02d:%02d]", on_level_to_str(l),
