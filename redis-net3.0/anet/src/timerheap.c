...
int th_init(int capacity){
    T = zmalloc(sizeof(timer_heap_t));
    T->array = (th_timer_t **)zmalloc(capacity * sizeof(th_timer_t));
    T->size = 0;
    T->capacity = capacity;
    return 0;
}
...
th_timer_t*  th_add_timer(long long timeout ,time_cb cb,void *user_data) {
    assert(T);
    th_timer_t* timer =  zmalloc(sizeof(th_timer_t));
    timer->cb_func =cb;
...
void _th_resize() {
    T->array = (th_timer_t **)zrealloc(T->array, T->capacity * 2 * sizeof(th_timer_t));
    assert(T->array);
    T->capacity *= 2;
}
