local crctab = { 0x00000000, 0x77073096, 0xee0e612c, 0x990951ba, 0x076dc419, 0x706af48f,  
...
function crc32.hash(key)  
	local count = string.len(key)
    local crc = 0 ~ 0xFFFFFFFF
    
	for i=1,count do
		local byte = string.byte(key, i)
		local a = (crc>>8) & 0x00FFFFFF
		local b = crctab[((crc~byte) & 0xFF)+1]
		crc =  (a ~ b)
    end
